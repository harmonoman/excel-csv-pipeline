...
# Fields that must be present as keys in mapping.json["fields"].
# Note: "Client" is intentionally excluded — it is derived from the sheet tab name
# at runtime by inject_client() and is never a source column in the Excel file.
# The validator's REQUIRED_FIELDS list includes Client because it must be present
# in the final DataFrame, but it does not need an alias entry in mapping.json.
REQUIRED_FIELDS = [
    "First",
    "Last",
    "Address1",
