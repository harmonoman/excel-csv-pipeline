...
WHY MAPPING HAPPENS HERE (NOT IN PIPELINE):
When sheets with different column aliases are concatenated first and mapped
second, pandas creates a union of all source column names. The mapper then
only picks one alias winner per canonical field — rows from sheets that used
a different alias end up with NaN for every affected field.

By mapping each sheet individually before concat, all sheets arrive at concat
with identical canonical column names, so the union is clean and complete.
...
def parse_workbook(
    ...
    Each sheet is:
    1. Read once (rows materialized to avoid read_only generator exhaustion)
    2. Header detected
    3. Built into a DataFrame with _source_sheet
    4. Column-mapped to canonical names BEFORE concatenation

    Mapping per-sheet before concat ensures sheets with different column
    aliases all produce identical canonical column names, preventing NaN
    leakage when the DataFrames are concatenated.
    ...
    Returns:
        Single concatenated DataFrame from all valid sheets with
        canonical column names. Empty DataFrame if no valid sheets found.
    """
    ...
    for sheet_name in wb.sheetnames:
        ...
        # Map columns to canonical names per sheet BEFORE concatenation.
        # This ensures sheets with different aliases all arrive at concat
        # with identical column names — preventing NaN leakage.
        df = map_columns(df, mapping_config)
